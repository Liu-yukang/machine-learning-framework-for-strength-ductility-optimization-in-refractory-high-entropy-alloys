import pandas as pd
import numpy as np
from sklearn.ensemble import (Random    RandomForestRegressor, GradientBoostingRegressor,
...
data = pd.read_excel(r'C:\Users\Liu\Desktop\YS-date.xlsx').loc[:, ~data.columns.duplicated()]
target = "Ys"
...
voting_model = VotingRegressor([
    ('RF', base_models['RF']), ('XGBT', base_models['XGBT']),
    ('LGBM', base_models['LGBM']), ('GBDT', base_models['GBDT'])
])

stacking_model = StackingRegressor([
    ('RF', base_models['RF']), ('XGBT', base_models['XGBT']),
    ('LGBM', base_models['LGBM']), ('SVC.rbf', base_models['SVC.rbf'])
...
for _ in tqdm(range(num_iterations), desc="进度"):
    ...
    for name, model in models.items():
        ...
        for tr_idx, te_idx in kf.split(X):
            ...
            y_pr = model.predict(X_te_scaled)
...
print(f"\n模型性能（100次五折交叉验证）：")
print(f"特征数: {len(features)}, 样本数: {len(data)}")
print(f"目标“{target}”方差: {y_var:.4f}")
print("-" * 100)
print(f"{'模型':<12} | {'RMSE均值':<10} | {'RMSE标准差':<12} | {'R2均值':<10} | {'R2标准差':<12}")
print("-" * 100)

for name, met in sorted(results.items(), key=lambda x: -np.mean(x[1]['R2'])):
